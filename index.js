document.addEventListener('click',e =>{
  if(e.target.className!=='tex'){return}
  qwe=e.target.id
...
function add_nap(){
    ...
    drink2.id=`id ${count}`
    drink2.querySelectorAll('.selec').forEach((milk)=>{milk.name=`selec${count}`})
    drink2.querySelector('.tex').id=`text${count}`
    drink2.querySelector('.tex').name=`te${count}`
    drink2.querySelector('.pojel').id=`poj${count}`
    drink2.querySelectorAll('.rad').forEach((milk)=>{milk.name=`rad${count}`})
